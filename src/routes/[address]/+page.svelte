<script lang="ts">
  ...
  let sorting = {
    f: (t: Transaction) => Number(transactionFee(t) / BigInt(1e6)),
    ascending: false,
  };
...
<div class="min-h-screen bg-cyan-100 px-10 py-4">
  <div class="max-w-screen-2xl mx-auto">
    <Search address={$page.params.address} {chain} />
      ...
      >
        <div class="bg-white rounded-2xl px-8 py-6 shadow-lg">
          Your top transaction took Ξ<span class="font-mono"
            >{(
              Number((stats.topEtherUsed.gasUsed * stats.topEtherUsed.gasPrice) / BigInt(1e6)) /
              1e12
            ).toFixed(6)}</span
          >,
          {(
        ...
        <div class="bg-white rounded-2xl px-10 py-8 shadow-lg text-lg">
          ...
          >, at an average gas price of
          <span class="font-mono">{stats.averageGasPrice.toFixed(3)}</span>
          gwei.
        </div>
        <div class="bg-white rounded-2xl px-8 py-6 shadow-lg">
          ...
          <thead class="bg-gray-800 text-white">
            <tr>
              <th
                ...
                on:click={sortBy((t) => t.timestamp.getTime())}>Timestamp</th
              >
              ...
              <th
                class="text-left py-3 px-4 uppercase font-semibold text-sm"
                on:click={sortBy((t) => Number(t.value / BigInt(1e6)))}>Value</th
              >
              <th
                class="text-left py-3 px-4 uppercase font-semibold text-sm"
                on:click={sortBy((t) => Number(t.gasUsed))}>Gas Used</th
              >
              <th
                class="text-left py-3 px-4 uppercase font-semibold text-sm"
                on:click={sortBy((t) => Number(t.gasPrice))}>Gas Price (GWEI)</th
              >
              <th
                class="text-left py-3 px-4 uppercase font-semibold text-sm"
                on:click={sortBy((t) => Number(transactionFee(t) / BigInt(1e6)))}>TX. Fee</th
              >
              <th
                class="text-left py-3 px-4 uppercase font-semibold text-sm"
                on:click={sortBy((t) => Number(transactionFee(t)) / Number(t.value))}
                >Value Percentage</th
              >
              <th
                class="text-left py-3 px-4 uppercase font-semibold text-sm"
                on:click={sortBy(
                  (t) => Number(transactionFee(t)) / 1e18 / Number(stats.totalEtherUsed),
                )}>Total Fee Percentage</th
              >
            </tr>
          </thead>
          <tbody class="text-gray-700">
            {#each sortedTransactions as transaction, i}
              <tr class:bg-gray-100={i % 2 === 0}>
                ...
                <td class="text-left py-3 px-4 font-mono"
                  >{(
                    (Number(transactionFee(transaction)) / Number(transaction.value)) *
                    100
                  ).toFixed(2)}%</td
                >
                <td class="text-left py-3 px-4 font-mono"
